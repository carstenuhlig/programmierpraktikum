...
#print $num[0];
if($num[0] < 1){
$seq_query = $dbh->prepare("INSERT INTO sequences \(sequence, organism\) VALUES\(\'$genseq\', \'$organism\'\)");
$seq_query->execute();
}
...
for($i = 0; $i<=$#ps; $i++){
	...
	#print $num[0];
	if($num[0] < 1){
		$seq_query = $dbh->prepare("INSERT INTO sequences \(sequence, organism\) VALUES\(\'$protseq\', \'$organism\'\)");
		$seq_query->execute();
	}
}
